class InciventsController < ApplicationController
  ...
  def show
    @account = current_user.account
    deny_access_wrong_account unless @incivent = @account.account_incivents.incivents_from_groups_joined_by(current_user).find(params[:id])
  end

  def edit
    if (current_user.master_user? && current_user.account == Incivent.find(params[:id]).group.account ) || current_user.incivents.find(params[:id])
      @account = current_user.account
  ...
  def create
    ...
    if @incivent.save
      @incivent.group.users.each do |user|
        Notification.notify_incivent(user, group, incivent_url(@incivent)).deliver rescue logger.error 'Unable to deliver the event email.'
        group.updates_add_create(group.name, 'incivent', @incivent.name)
      end
  ...
  def update
    ...
    if @incivent.update_attributes(params[:incivent])
      group.updates_add_archive(group.name, 'incivent', @incivent.name) if @incivent.archive == true
  ...
  def destroy
    ...
    group = incivent.group
    if incivent.destroy
      group.updates_add_delete(group.name, 'incivent', incivent.name)
