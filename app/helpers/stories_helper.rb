module StoriesHelper
 ...
 def stories_lats(stories)
  ...
  stories.each do |story|
    latarray.push(story.latitude) unless story.cannot_geolocate
  end
 ...
 def stories_longs(stories)
  longarray = []
  stories.each do |story|
    longarray.push(story.longitude) unless story.cannot_geolocate
  end
 ...
 def stories_names(stories)
  ...
  stories.each do |story|
    namearray.push(
      "<div class='label-story'><strong>Story</strong></div>
 ...
 def stories_urls(stories)
  urlarray = []
  stories.each do |story|
    urlarray.push(story_url(story)) unless story.cannot_geolocate
  end
